...
- [~] `/programs/private` — verify pricing 800 THB solo / 600 THB group (3 max), slots now 9 (7:00, 8:00, 10:00–16:00). Kroo → Kruu spelling normalized 2026-04-16. Trainer name list in page needs sync with rebuilt `/team`. **Wave 6 done 2026-04-17**: 12h cutoff enforced only on same-day bookings (interpretation B — future dates always bookable online). Greyed-out slots + WhatsApp fallback box with pre-filled message when in cutoff. Server-side check in `/api/checkout`. Helpers `isSlotWithinCutoff`, `buildWhatsAppUrl`, `CAMP_WHATSAPP_*` in `src/content/schedule.ts`. Admin bypass intentional. **Timezone bug fixed**: `AvailabilityCalendar` + 4 wizards now use `format()` from date-fns for `start_date` / `end_date` instead of `toISOString().split("T")[0]`, which silently skipped a day in positive timezones (e.g. GMT+1/+7 user selecting 18 April would send "2026-04-17" to the API and see today's block grey out tomorrow's slot). **Known open point**: private slots treat each `private-slot` block as blocking for everyone; in reality the camp has several trainers so capacity > 1 per slot should be allowed. To discuss/implement separately.
...
- [~] `/pricing` — Wave 1 done 2026-04-17: "7 days" → "6 days", "3 people/kids max" on small group private. Wave 2 done 2026-04-17: resident-10 + resident-20 Stripe products archived, new `resident-monthly` (3,000 THB / 1x/day) created, 3 DTV products created (`dtv-6m-2x` 20K, `dtv-6m-4x` 25K, `dtv-6m-unlimited` 33K). `src/content/pricing.ts` updated with new entries, BookingType/PriceCategory enums extended with `dtv`, Zod schemas updated. `/pricing` now shows single Resident card + new DTV section. **Wave 4 pending**: justify fighter 9,500 THB discourse.
